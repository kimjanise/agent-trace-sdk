...
import { Trace, ToolExecution, TreeNode, LLMCall, STTCall, TTSCall } from "@/types/trace";
import {
  X,
  ChevronUp,
  ChevronDown,
...
import { getLLMCalls, getToolExecutions, getSTTCalls, getTTSCalls } from "@/lib/supabase";
...
function buildTree(
  ...
  // Combine all items with their timestamps for sorting
  type TimelineItem =
    | { type: "llm"; data: LLMCall }
    | { type: "tool"; data: ToolExecution }
    | { type: "stt"; data: STTCall }
    | { type: "tts"; data: TTSCall };

  const timeline: TimelineItem[] = [
    ...llmCalls.map((d) => ({ type: "llm" as const, data: d })),
    ...toolExecutions.filter((t) => !t.llm_call_id).map((d) => ({ type: "tool" as const, data: d })),
    ...sttCalls.map((d) => ({ type: "stt" as const, data: d })),
    ...ttsCalls.map((d) => ({ type: "tts" as const, data: d })),
  ];

  timeline.sort(
    (a, b) => new Date(a.data.started_at).getTime() - new Date(b.data.started_at).getTime()
  );
...
export default function TraceDetailPanel({
  ...
  const [sttCalls, setSTTCalls] = useState<STTCall[]>([]);
  const [ttsCalls, setTTSCalls] = useState<TTSCall[]>([]);
  ...
  useEffect(() => {
    async function loadTraceDetails() {
      ...
      try {
        ...
        setSTTCalls(sttCallsData);
        setTTSCalls(ttsCallsData);
  ...
  // Handle error selection from header - focus on error node
